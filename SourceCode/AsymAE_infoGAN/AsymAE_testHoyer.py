...
def test_Hoyer_Asym_AE(AsymAE_class=Sparse_NonNeg_AsymAEinfoGAN_Hoyer_NonNegConstraint, sparsity_weights = [1], sparsity_objectives = [0.6], latent_dimension=100, nb_epochs=500, 
    ...
    for idx1, sp_w in enumerate(sparsity_weights):
        for idx2, sp_o in enumerate(sparsity_objectives):
            ...
            train_Hoyer_loss[idx1, idx2] = shAE.total_loss(x_train, X_rec_th=original_images_train) - train_rec_errors[idx1, idx2]
            test_Hoyer_loss[idx1, idx2] = shAE.total_loss(x_test, X_rec_th=original_images_test) - test_rec_errors[idx1, idx2] 
            ...
            np.save(out_path +'_training_Hoyer_loss_' + strDims, train_Hoyer_loss)
    ...
    if svm:
        results = pd.DataFrame(data={'sparsity_weight':np.repeat(sparsity_weights,len(sparsity_objectives)), 'sparsity_objective':np.tile(sparsity_objectives, len(sparsity_weights)),
                                ...
                                'training_Hoyer_loss':train_Hoyer_loss.flatten(), 'test_Hoyer_loss':test_Hoyer_loss.flatten(),
