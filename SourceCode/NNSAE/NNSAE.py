...
class NNSAE:
    ...
    def __init__(self, inpDim, hidDim):
        ...
        self.inp =  np.zeros((self.inpDim,1))   #vector holding current input
        self.out =  np.zeros((self.inpDim,1))   #output neurons
        self.g =  np.zeros((self.hidDim,1))     #neural activity before non-linearity
        self.h =  np.zeros((self.hidDim,1))     #hidden neuron activation
        self.a =  np.ones((self.hidDim,1))     #slopes of activation functions
        self.b = -3* np.ones((self.hidDim,1))     #biases of activation functions
        scale = 0.025
        
        self.W = scale * (2 * np.random.rand(self.inpDim, self.hidDim) - 0.5 *  np.ones((self.inpDim, self.hidDim))) + scale     #shared network weights, i.e. used to compute hidden layer activations and estimated outputs
        print(self.W.shape)
    ...
    def train(self, X):
        ...
        p = np.random.permutation(range(numSamples)) #randperm(numSamples); #for randomized presentation
        for i in range(numSamples):
            #if (i % 1000) == 0:
            #    print('Sample ', i)
            #set input
            self.inp = np.array([X[p[i],:]]).T            #forward propagation of activities
            self.update()
            ...
            #update weights
            self.W = self.W + lrate[0] * np.multiply(error, self.h.T)

            #decay function for positive weights
            if self.decayP > 0:
                self.W[self.W > 0] -= self.decayP * self.W[self.W > 0]
            ...
            if self.decayN == 1:
            	...
            	if self.decayN > 0:
            		self.W[self.W < 0] -= self.decayN * self.W[self.W < 0]
            
            
            #intrinsic plasticity
            hones =  np.ones((self.hidDim,1))
            tmp = self.lrateIP * (hones - (2.0 + 1.0/self.meanIP)) * (self.h + np.power(self.h, 2)/self.meanIP)
            self.b = self.b + tmp
            self.a = self.a + self.lrateIP * hones / self.a + np.multiply(self.g, tmp)
    ...
    def update(self):
        #excite network
        self.g = np.dot(self.W.T, self.inp)
