...
class AsymAE:
    ...
    @classmethod
    def load(cls, model_name, custom_objects={}, path_to_model_directory="../AsymAE/"):
        """
    ...
    def train(self, X_train, nb_epochs=100, X_val=None, verbose=1):
        if X_val is None:
            ...
            validation_data = (X_val, X_val[:,:,:,0])
        
        self.autoencoder.fit(X_train, X_train[:,:,:,0],
                epochs=nb_epochs,
                batch_size=128,
                shuffle=True,
    ...
    def save(self, path_to_model_directory="../AsymAE/"):
    ...
    def reconstruction_error(self, X_test,f):
        return self.autoencoder.evaluate(X_test, X_test[:,:,:,0], verbose=0)
