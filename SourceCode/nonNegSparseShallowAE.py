...
class Sparse_NonNeg_ShallowAE_Hoyer_NonNegConstraint(ShallowAE):
    """
    Auto-Encoder with sparse and Non-Negativity constraintes (Hoyer constraint for sparsity, Keras NonNeg constraint (projection) for Non Negativity)
    Attributes: 
        ...
        sparsity_objective: float between 0 and 1 - the objective for the value of the Hoyer sparsness metric (the higher the sparser).
    """
    def __init__(self, latent_dim=100, nb_rows=28, nb_columns=28, nb_input_channels=1, one_channel_output=True, 
        ...
        encoded = Dense(latent_dim, activation='sigmoid', 
                            activity_regularizer=custom_regularizers.sparsity_Hoyer(beta=self.sparsity_weight, sp_o=self.sparsity_objective))(x)
    ...
    def load(cls, model_name, custom_objects={}, path_to_model_directory="../Results/ShallowAE/"):
        ...
        loaded_AE.sparsity_objective = loaded_AE.encoder.get_config()['layers'][2]['config']['activity_regularizer']['config']['sp_o']
        return loaded_AE
