...
def test_KL_div(sparsity_weights = [1], sparsity_objectives = [0.1], latent_dimension=100, nb_epochs=200, svm=False, path_to_dir = "../ShallowAE/"):
    ...
    max_approx_error_train = np.zeros((nb_sparsity_weights, nb_sparsity_objectives))
    max_approx_error_test = np.zeros((nb_sparsity_weights, nb_sparsity_objectives))
    ...
    for idx1, sp_w in enumerate(sparsity_weights):
        for idx2, sp_o in enumerate(sparsity_objectives):
            ...
            max_approx_error_train[idx1, idx2] = shAE.max_approximation_error(x_train, morphoMaths.dilatation, apply_to_bias=True, SE_scale=1)
            max_approx_error_test[idx1, idx2] = shAE.max_approximation_error(x_test, morphoMaths.dilatation, apply_to_bias=True, SE_scale=1)
            ...
            np.save(out_path +'_training_max_approx_error_dilatation' + strDims, max_approx_error_train)
            np.save(out_path +'_test_max_approx_error_dilation' + strDims, max_approx_error_test)
            if svm:
    ...
    if svm:
        results = pd.DataFrame(data={'sparsity_weight':np.repeat(sparsity_weights,len(sparsity_objectives)), 'sparsity_objective':np.tile(sparsity_objectives, len(sparsity_weights)),
                                ...
                                'training_max_approx_error_dilatation':max_approx_error_train.flatten(),
                                'test_max_approx_error_dilatation':max_approx_error_test.flatten(),
